...
- **系统板块与板块行情**（`system_blocks` / `board_*`）：行业/概念板块发现、成分股、
  板块指数行情/分时/竞价/**日K**（16384 → 0x42 表，2026-09-09 打通），板块通道走
  fu4，成分股走股票行情网关。成分股支持**板块指数代码**（882/885/886xxx）：本地
  BlockUpdate 只有 90 个 881 行业同号命中，概念/二级行业按 name_48 名称桥接到
  十六进制 block_id 展开（实测 513/513 全覆盖）。沪市 ST 股（`*ST`/`ST`）在
  8901 全车道使用风险警示板市场码 **22**（MAIN/L2/kline/分时/十档/逐笔/4214
  注册均已支持，活网对照 CodeList=17 超时、22 返回）。
...
（默认 `127.0.0.1:8765`）提供数据，四个视图经顶栏或 URL `?view=` 切换：
...
- **94板块**（`?view=boards`）：2 行 × 3 列——(1,1) 热点板块列表
  （513 个，表头排序）、(2,1) 板块成分股（涨幅列随查询自带，到达即排；
  表头排序跨板块/跨刷新记忆）、(1,2/1,3) 板块分时 + 板块日K（蜡烛 +
  MA 预热 + 范围记忆）、(2,2/2,3) 个股分时 + 日K（复用全局股票上下文，
  点击成分股联动）。三列宽度可拖拽并持久化；点击板块自动选中当前排序
  首行（4 秒跟随窗口，盘中不横跳）；板块选择写入 URL `?board=`。
