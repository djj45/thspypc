...
>
> 最近能力盘点：2026-07-28（代码基线 `113df10`，状态随本文后续实现更新）。
...
| 模块 | 方法 | 现状 | 缺什么 |
|------|------|------|--------|
| 历史分时（回忆） | `history_timeline` | 指数定长帧（1A0002 类）可解 | 个股帧还包含字段头/字段表稀疏打包，不能仅按 93/94 字节切行；四日 thsdk 真值已建立，见 `HISTORY_TIMELINE_VARLEN_INVESTIGATION.md` |
...
| 🔴 P0 | **个股历史分时变长帧** | 已公开方法的正确性缺口，优先于继续扩大 API 面 |
