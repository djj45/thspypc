...
|--------|------|------|
| 申万一/二/三级行业板块列表 | ❌ | 无 |
| 同花顺行业板块列表（881xxx 指数） | ✅ 活网通过 | 本地 oracle 已有 881xxx→名称/成分股（~90 个）；8901 的 0x130 板块指数请求/响应已通过双账号活网验证（成分股另见下项） |
...
| 板块行情（指数/涨跌幅排行） | ✅ 活网通过 | `client.board_quotes()`（0x130 板块行情：代码+GBK 名称+OHLC） |
...
- 8901 排障方法论与本次成分股故障的完整复盘见
  `docs/POSTMORTEM_CONSTITUENT_20260801.md`（新通道先核对对端后端，再看逐帧
  服务端响应，最后才做字节对照）。
