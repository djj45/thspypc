...
本文记录 thspypc 已观察到的服务器拓扑。域名来自 HTTP 鉴权响应中的
...
| MAIN | `ifindhq.123ths.com:8901` | 普通登录；`VerifyCode=0` 后不发送带 `MarketCode` 的 L2 init | `BASIC_QUOTE`；不是 L2 专用 | `stock_list`、热门/排序列表、批量行情、基础分时、K 线、名称增量等 MAIN 请求 |
...
| REALORDER | 当前实现固定 `106.14.65.90:9601` | 独立 9601 登录 | `REALORDER`；这是独立能力，不能仅由“是否 Level2 账号”推断 | `qurealorder` 历史异动、`subrealorder` 实时异动订阅、`pushrealorder` 接收 |
...
- 不要把带 `MarketCode` 的 L2 init 发到 MAIN。部分 MAIN 实例会直接关闭连接。
