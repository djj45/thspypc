...
|--------|------|------|
| 申万一/二/三级行业板块列表 | ❌ | 无 |
| 同花顺行业板块列表（881xxx 指数） | ✅ 活网通过 | 本地 oracle 已有 881xxx→名称/成分股（~90 个）；8901 板块指数请求/响应已通过双账号活网验证（成分股另见下项）。2026-08-02 修订：列表请求路由改为 0x006C/0x016C（普通）/0x0052/0x0152（L2）、无 history flag、LackTime 全 0，响应为 0x20/0x1c/0x22 紧凑表（见 HANDOFF_SYSTEM_BLOCKS_CAPTURE_20260801.md；08-01 旧形态待另一台电脑验证） |
