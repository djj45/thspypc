...
- **2026-08-08 补充**：名称同步逐组“登录→下载→断开”串行重登也会触发 -1；同花顺冷启动
  是同时建 8 条 8901 连接并发登录，之后长驻 + 3s 心跳复用。`download_all_stock_names`
  已按此实现（并发登录 + 发送锁 + 心跳线程），批量刷新走 `THSClient.fetch_all_stock_names()`。
- 与 2026-07-23 的 login 帧内容原因（sk/sv、动态 check 字节）互相独立：先确认帧字节，
  再排查连接频率。
...
**深市历史尾盘（已打通，2026-07-30）**：
深市（`000001`/`000938`/`300033`）对 `pageid=4417` 历史尾盘请求参数
与沪市**完全同构**（`33(000001,)` + `DataType=10,49,287` + `7424` +
`4417`），由同一 `build_l2_closing_auction_query` + 解析器处理，无市场
分支。深市 tick 间隔 9 秒（≈20-21 点），沪市 3 秒（≈61 点）——这是真实
市场节奏差异，非 bug。早期"深市返回 ServerCost=257/514 拒绝帧"是
`probe_l2_closing_variants.py` 的收帧 bug：它把深市第一个短确认帧（26/42B，
含 ServerCost 文本字段）当成最终结果，没继续读后面的真实数据帧。深市
数据帧同样是 hd1.0 + 末字节截断，上一轮的 `_decode_closing_rows` 容错
修复直接适用。深市必须连 szlv2 节点（DNS `szlv2.123ths.com`，9 个 IP）；
MAIN 节点对 4417 只回短帧。

活网回归（`8.134.99.165` szlv2 节点，fresh login）：000001=21 点
（价 10.88→10.89）、000938=19 点、300033=21 点（价 324.64），均
2026-03-11 14:57:00–15:00:00。新增离线回归测试
`test_closing_auction_parser_handles_sz_9s_stride`（9 秒步长 + 截断），
全套 314 测试无回归。
