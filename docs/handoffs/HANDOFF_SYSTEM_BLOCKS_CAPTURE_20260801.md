...
2. **普通账号 9355 历史分时是稀疏格式**：一天的行按乱序分段存放在同一解压
   响应体的多张 0x42 表里，且只含部分分钟（07-23 201/241、06-30 185/241）。
   缺失分钟在 L2 全量数据中都有真实值（价格在变化），因此是服务端普通通道
   稀疏下发，不是客户端缺分钟。解析器已支持多表合并 + 乱序锚定
...
3. **L2 4417 历史分时响应完整 241 点**，且响应体含两张表：0x007E（基准
   ...
   `history_000938_20260723_115059.bin`（43,888B），已纳入四日期回归。
