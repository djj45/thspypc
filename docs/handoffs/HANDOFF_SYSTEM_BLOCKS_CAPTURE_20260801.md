...
## 2026-08-02 补充：板块列表请求形态漂移（08-01 本机抓包实证）

### 现象与修复

`system_blocks_protocol.py` 里 0x130 列表行情请求此前用的形态在 **2026-08-01
本机抓包**中即可找到（`captures_live/system_blocks_20260801_*.pcap`）：
...
对该形态**静默不回复**（另一台电脑重放本机 0x0139 请求实测无数据）。对照
08-02 真实客户端请求帧（`tests/fixtures/board/list_req_392.bin`，逐字节
一致），列表查询路由已改变：

| 字段 | 08-01 本机抓包真值 | 08-02 抓包真值 |
|---|---|---|
...
| 前缀可见码数 | 36 | 51 |
| 响应表型 | 0x20/0x1c/0x22 紧凑表 + 0x130 名称行情表（344B/行，含 GBK 名称） | 0x20/0x1c/0x22 紧凑表（513 行，无名称列） |

> 字节级证据（2026-08-02 复核本机 pcap）：
...
> - 0x130 名称表（hd3.1 表类型 0x130、344B/行、72 字段）在 08-01 本机 4 个
>   pcap 中均被服务端推送（13:23 L2 抓包 8.82s 起共 103 次，约 0.2~3s 一次，
>   为打开板块列表页后的周期推送，非严格一问一答）；`_board_quote_0x130.bin`
>   可解出 17 行（881101 种植业与林业 … 881124 消费电子）。
...
- 08-01 抓包在本机完成，0x0039/0x0139 形态当时被服务端正常受理（回
  0x20/0x1c/0x22 紧凑表 + 0x130 名称表）；早期文档“旧形态来自另一台电脑、
  另一套 hexin 版本”的归因不准确，已按本机 pcap 逐字节证据更正。
- 08-02 起服务端对该路由形态静默不回复：另一台电脑重放本机 0x0139 请求
  无数据，且该电脑客户端抓包不再出现 0x0139 请求（当前客户端改发
  0x006C/0x016C）。结论为**服务端协议变化**（0x0139 路由已弃用），而非
  客户端版本差异。
