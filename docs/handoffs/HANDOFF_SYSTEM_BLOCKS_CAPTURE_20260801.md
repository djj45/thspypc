...
### 2026-08-03 交易日同环境受控复验：旧路由仍被受理（结论更新）

用 `tests/_probe_board_route_compare.py` 在**同一板块连接**上做受控对照
（把当前列表帧的 route 字节改回 0x0039/0x0139，其余字节——无 history
flag、LackTime 全 0、seq、universe 文本——完全一致），普通账号
（.env.normal）与 L2 账号（.env）各复验一次，均发生在交易日 2026-08-03：

- 旧路由 0x0039/0x0139：均返回 0x20/0x1c/0x22 紧凑表 **513 行**，命中
  881101/881121/885480；响应帧头回显请求 route=0x0139（普通）与
  0x0139（L2 探针同样被受理）。
- 当前路由（普通 0x006C/0x016C、L2 0x0052/0x0152）：返回逐字节相同的表。

结论边界更新：

- 08-02 记录的“旧路由跨机重放无数据”在当前环境**未复现**：同一引导、同一
  会话身份下旧路由仍被受理并返回相同紧凑表，因此“纯服务端弃用旧路由”不能
  作为已闭环事实；08-02 重放无数据更可能是重放连接缺少完整引导/会话身份
  （与 08-01 成分股空列表同型）或节点差异。
- 发送侧保持 08-02 形态不变（生产路径与最新抓包一致），旧路由不设为兼容
  回退也不删除。
- 本复验在**交易日**进行；非交易日旧路由/当前路由的返回行为未验证，后续若
  在非交易日观察到差异需单独记录，不能据此反推协议变化。
- 注意：08-02 的“旧路由跨机重放无数据”观测发生在**周末（周日）**，而
  08-01（周六）跨机抓包中旧路由当时可用——周末两天结论本身就不一致。用户
  提示旧路由可能是**盘中推送/盘中可用路径**，与交易日状态相关。计划：
  08-04（周二）开盘时段用 `tests/capture_system_blocks.py` 抓真实客户端
  （分析已加【4b】路由分布，直接区分 0x0039 旧路由与 0x006C/0x0052 新
  路由），同时跑 `tests/_probe_board_route_compare.py` 双账号对照；若开盘
  时段旧路由与收盘/周末行为不同，再以此为准更新本文结论。
- 0x130 归因结论不变（板块详情相关板块组件周期推送）。
