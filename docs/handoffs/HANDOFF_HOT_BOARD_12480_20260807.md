...
## 活网验证（2026-08-07 晚，普通账号 mx_722141944）

- `hot_boards([...885927])`：885927 chg=+8.05%、涨停=8、涨家=73、跌家=3、
  4分钟涨速=-0.003 —— 与 94 页面 UI 逐项一致 ✅
- `hot_boards(None)`：513 个板块全量（详情+527527 合并）✅
- `hot_boards_sorted(199112)`：涨幅降序，885927=8.05% 排第 2 ✅
- `hot_boards_sorted(592890)`：主力净流入，885338=408.64亿 ✅
- `hot_boards_sorted(271)`：涨停数，885338=56 ✅

## 遗留项（下轮，非阻塞）

1. **L2 账号 0x118/215B 大表**（dt180/179/178 等，Level2 成分股/板块行情）：
   本轮仅做离线识别，未接入。L2 活网验证 `hot_boards`/`hot_boards_sorted`
   时确认基础字段（0x40/0x20 表）可用即可，0x118 全字段另需对照 UI 锚定。
2. **成分股页面**（板块个股列表）走 `HOT_STOCK_ROUTE`（0x003B/0x013B、
   0x0054/0x0154），CodeList 用股票市场码 17/22/33/151；本轮已验证个股
   行情请求形态存在，未单独封装 `hot_constituents`。
3. 竞价涨幅（开盘价/昨收 间接计算）与封单额、竞价金额（已实现表头同款
   间接计算）无需协议改动，客户端本地计算。
