...
当前目标是提供可用的**服务器排名数据 API**，该目标已经由 `dde_rank()` 完成。
以下行为只在真正开发 DDE 前端时才需要，不再列为当前后端 P0：

- `DDESession` 排名分页缓存和沪深合并状态；
- 可见窗口的 `AddCode/DelCode` 差分订阅；
- 最新价、涨幅、成交额等行情字段实时刷新；
- 表头切换后的回滚、跳页、旧响应隔离和缓存失效；
- Level2 超过当前抓包所见 928 条后的加载策略；
- 交易时段中的动态重排、停牌股票和 `None` 值位置。

未来做前端时，应由后端维护长生命周期 `DDESession`，前端只发送排序条件和可见
范围，并通过 WebSocket/SSE 接收快照与增量。不要让前端直接管理 8901 socket，
也不要为翻页或表头切换重新登录服务器。

尚未确认的列语义也延期到对应前端列实际需要时验证：`592889 → dt249` 的页面
列名，以及成交量 `SortBy=13` 是否支持同一排名路径。成交额不再属于缺口：抓包已
确认 `SortBy=19 → dt19`。
