...
>   `LackTime=0,0,0,0,0,0,0,0`，seq=0x01C4/0x0068；旧形态
>   （0x0039/0x0139 + history flag + LackTime=0,3,…，08-01 另一台电脑抓包）
>   服务端静默不回复。响应为 0x20/0x1c/0x22 紧凑表（513 行，无名称列），
>   不再是 0x130 名称表；`parse_board_quote_response` 仅作兼容保留。
...
>   h17=0x1C，LackTime 全 0）；与 08-01 双子帧形态的差异待另一台电脑验证。
