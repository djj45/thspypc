...
## 2026-07-23 晚新增（沪深A股全市场列表界面 + list_quotes 字段含义实测确认）

### 13. 沪深A股全市场列表界面（✅ `tests/web_stocklist.py`，复刻同花顺）

复刻同花顺「沪深A股」列表界面（5500+ 只全量 + 表头点击排序）。

**数据源**：读同花顺本地名称缓存（`load_hexin_names()`）→ **5556 条沪深 A 股**
（沪 2459 + 深 3097，正好用户要的 5500+），瞬时零网络、稳定可靠。
**不用 `stock_list_cached()`**——其重放机制本会话实测不稳定（一次 0 条、一次 489 条，
全量帧 dc≈7422 的服务器实例可能不响应）。行情用 `list_quotes` 后台分批回填
（每批 30，按市场码 17/33 分组，186 批），渐进填充。

**界面特性**（纯内置 `http.server` + 原生 JS，零框架依赖，与 `web_dashboard.py` 同风格）：
- 虚拟滚动（视窗渲染 + 上下缓冲，5556 行流畅）
- 表头点击本地排序（升降序切换，默认成交额降序，null 恒排末尾）
- 同花顺深色风格，成交额(亿/万)/成交量(万手) 格式化，进度条 + 手动刷新

**用法**：
```bash
uv run python tests/web_stocklist.py
uv run python tests/web_stocklist.py --port 8888 --batch 50
```

**待办**：涨跌幅/振幅/换手率列暂不展示（需抓包确认服务器直接给的字段，见 §14），
表头已预留位置待补。

### 14. list_quotes DataType 字段含义（✅ 2026-07-23 实测确认）

实跑 `list_quotes`（600519/000001/600000）逐字段确认含义（此前 README 表格与代码
注释有不一致）：
...
| dt9 | 最低 | 1285.43 |
| dt10 | 最新价/现价 | 1292.01 |
| dt13 | **成交股数**（÷100=手） | 3391769 |
| dt18 | 委托量（待确认） | 26808 |
| dt19 | **成交额(元)**（服务器直接返回） | 4392505600 |
| dt48 | 涨速/4分钟涨幅 | -0.553 |
| dt49 | 竞价委托笔数（盘外固定 100） | 100.0 |
| dt66 | 涨幅（**盘中有效**，盘外=0） | 0.0 |

**关键纠正**：
- **dt13 单位是股，不是手**。3391769 ÷ 100 = 33917.69 手 ≈ 33918 手（与同花顺客户端
  显示完全一致，差 31 是四舍五入进位）。README 旧注释"成交量(dt13)"未标单位，易误读。
- **dt19 直接是成交额**（元），不用 `dt13 × dt10` 算。README "成交额 = dt13 × dt10"
  是早期未确认时的写法，应删。
- **涨跌幅**：dt66 盘外为 0，盘中是否可靠需开盘抓包确认。未确认前用本地算
  `(dt10-dt6)/dt6`（dt6=昨收非 0 即可）最稳。

**待抓包补的列**（web_stocklist 表头已预留）：涨跌幅、涨跌额、振幅、换手率——
确认同花顺请求哪些字段、哪个是服务器直接给的准确涨幅后补上。

### 15. market_snapshot_with_quotes 字段映射 bug（✅ 已修复）

`client.py:market_snapshot_with_quotes()` 存在两个 bug（实测确认 `r.get('price')=None`）：
1. **字段名不匹配**：用 `r.get("price")`/`r.get("change_pct")` 等具名键，但
   `list_quotes` 返回 `dt10`/`dt66` 原始键 → 所有行情字段拿到 None。
2. **datatype 漏 dt6**：`[5,7,8,9,10,13,18,19,48,49]` 缺昨收（dt6），实测此集合下
   dt6=None，即使修了字段名涨跌幅也算不出。

**修复**（已在先前提交）：datatype 加 6，回填改用 `r.get("dt10")` 等原始键。

### 16. 本地名称缓存作全量数据源（✅ 比 stock_list 重放可靠）

`load_hexin_names()` 读 `D:\同花顺软件\同花顺\stockname\stockname_*.txt`，实测：
- 总名称 **59755 条**（沪深北 A 股 + 新三板 + 基金 + 指数 + 可转债等）
- 过滤出沪深 A 股（`market_from_code` 返回 17/33）→ **5556 条**
- 瞬时、零网络、稳定（读本地文件），远比 `stock_list` 重放（本会话拿 0~489 条）可靠

**结论**：凡需全量沪深 A 股 code+name 的场景（列表界面、批量行情查询分组等），
优先用 `load_hexin_names()` + `market_from_code` 过滤，不依赖 stock_list 重放。
=======
...
#### 14d. 响应格式（✅ 已破解——复用 0x1763410 位平面转置，无需动态调试）

K线响应是 **hd3.1 的 `flag=0x0042/0x0046` 变体**（区别于排序榜单的 `0x0100`）。
2026-07-24 破解：**解码核心与标准 hd3.1 完全相同**——直接复用从 hexin.exe
`0x1763410` 移植、对 0x0100 已 Unicorn 逐字节验证 100% 的位平面转置函数即可，
区别仅在帧头布局（多 26 字节壳头）。

> ⚠ **纠正旧认知**：本节早先版本（git commit 4dad004）误判 0x0042 需"动态逆向"，
> 根因是当时试的 4 种重组方案**全是字节级转置**（行主序/字段主序/整体列优先/字段内列优先），
> 而真正的解法是**位平面（bit-plane）转置**——这条路径对 0x0100 早已验证可用，
> 却被遗漏。`0x00` 占一半的字节分布恰恰是 bit-plane 编码的典型特征，当时也已观察到，
> 但没接上"复用 0x1763410"这一步。教训：跨变体复用已验证的解码核心应优先穷举。

**已确认的帧结构**（抓包 + BitRLE 解码 + 金融约束验证）：
```
hd3.1\0
+ dc(LE32)            ← 记录数（K线根数）
+ flag(LE16)          ← 0x0042（分笔/tick 类）/ 0x0046（OHLC K线）
+ hs(LE16)            ← 单条记录字节长度（= 字段表 width 累加）
+ fc(LE16)            ← 字段数
+ 字段表(fc×4B)       ← (dt, fmt, flags, width)
+ 26字节壳头           ← [0:2]市场码 + [2:4]0x0001 + [4]0x21 + [5:11]6B ASCII代码 + padding
+ BitRLE头(BE32=dc*hs) ← 大端，与标准 hd3.1 一致
+ BitRLE位流
```

⚠️ **头格式与 0x0100 不同**：0x0042/0x0046 没有 0x0100 的 `unk` 字段，`fc` 紧跟 `flag`；
字段表后多 26 字节壳头（含股票代码）。但 BitRLE 流 + 位平面转置用现有
`_decode_bitrle_0x13746d0` + `_transpose_bitplane_0x1763410` 直接解，**输出正好
dc×hs 字节**，再按字段表切分即得行主序记录。

**两种 flag 的字段表**（抓包实测，fmt=0x70=THS float, width=4）：

| flag | 字段表（dt 顺序） | 数据类型 | dc 示例 |
|------|------------------|---------|---------|
| **0x0046** | dt1, dt7, dt13, dt19, dt11, dt74, dt9, dt8（完整 OHLCV） | **OHLC K线** | 336(月K)/2147(日K) |
| 0x0042 | dt1, dt7, dt10, dt12, dt18, dt49, dt75（无 dt8/9/11） | 分笔/tick 类（非 OHLC） | 33 |

OHLC K线（0x0046）字段语义：dt1=时间、dt7=开、dt8=高、dt9=低、dt11=收、
dt13=量、dt19=额、dt74=复权因子/换手（末根有值）。

**时间字段（dt1）格式随周期而变**（抓包确认）：
- 分/日K：Unix 时间戳（如 `1784789709` = 2026-07-23 14:55:09）
- 周/月K：YYYYMMDD 紧凑整数（如 `19980831` = 1998-08-31，000089 上市月）
- 解析器用 `dt1 < 100_000_000` 判定 YYYYMMDD（恒 < 10^8），否则当 Unix 时间戳

**验证结果**（`tests/test_kline.py`，抓包 stream1 共 15 个 OHLC 帧）：
- **18473/18473 根通过金融约束**（high≥open/low/close，价格区间合理）= 100%
- 时间跨股票全历史（000089 月K 1998-08-31→2026-07-23；000069 日K 2017→2026）
- 少量前复权老票（000063/000062）OHLC 为负值是**前复权向下调整的正常现象**，
  非解码错误（约束检查因价格非正而计为未通过，但数值正确）

**时间字段 dt1 的三种编码**（2026-07-24 主动请求 + 抓包对照完全澄清）：

| K线类型 | dt1 编码 | 示例 | 还原 |
|---------|---------|------|------|
| 日K/分K | Unix 时间戳 | `1784789709` | 2026-07-23 14:55:09 |
| 周K/月K | YYYYMMDD 紧凑整数 | `19980831` | 1998-08-31 |
| **日内K（5分等）** | **绝对 bar 序号** | `132491944`（增量=5） | 不可直接还原，存 `bar_index` |

第三种（bar 序号）曾误判为"日K时间编码 bug"——实为日内K的 dt1 存连续 bar
编号（增量=周期分钟数），当 Unix 时间戳解会落到 1970 年代。解析器用
`_kline_dt1_is_bar_index`（首根值当 Unix 解出年份 <1990 即判定）检测，
此类帧 `time=None`、原值保留在 `bar_index` 字段。

**已知限制**：
- flag=0x0042 帧不全是 OHLC K线（部分是分笔/tick 类），解析器按字段表解码，
  仅当字段表含 dt7/8/9/11 时才填 open/high/low/close。

**已实现**：
- `parse_kline_hd3_response(body)`（src/thspypc/protocol.py）—— 响应解码，
  返回 `[{code, time, open, high, low, close, volume, amount, bar_index?, dt<N>...}]`。
- `build_kline_query(code, market, period, ...)` —— 请求构造（路由随周期）。
- `tests/probe_kline.py` —— 主动探测脚本（发请求拿响应，积累样本）。
- 均从 `thspypc` 包导出。**无需动态调试、无需绕反调试。**

**请求路由关键**（❗ 纠正 §14a 旧文档"路由0x0005"）：
抓包 16 个请求归纳，route=[11:13] 随周期：日K/分K=`0x0001`，**周K/月K=`0x014e`**
（必需！用 0x0001 服务器只回 26B ACK 不下发数据）。[18]=周期码高字节。
周/月K 部分服务器 IP 不支持 route 0x014e（直接 RST 断连），IP 轮换命中支持
的 IP 即可（实测 8.134.x.x / 139.9.x.x 等支持）。

**当前可用度**：请求 + 响应解码端到端验证通过（000089 日/周/月K 全部 100%
金融约束通过，时间编码正确）。client 层封装已加（见 §14f）。

#### 14f. 连接复用 + client.kline()（✅ 解决周/月K 断连，复刻 hexin 长连接）

**关键纠正**（2026-07-24）：抓包确认 hexin 的 16 个日/周/月/5分K 请求**全部走同一条
TCP 长连接**（`218.245.102.0:8901`，675 包），**不轮换 IP**。早先版本误判"周/月K 部分
IP 不兼容 route 0x014e 需轮换 IP"——实际是 thspypc 每次新建短连接发单个请求时
会话状态不稳定，长连接复用则稳定。

**已实现** `THSClient.kline(code, period, count, ...)`（src/thspypc/client.py）：
- **连接复用**：首次调用 connect()，后续调用复用 `self._sock`（复刻 hexin 单连接连发）。
  实测同一条连接连发 000089 日/周/月/5分K + 600000 日/月K 共 6 个请求全部成功。
- **自动重试**：连接断开或读超时时 `is_connected` 探测 → 不通则 connect()（IP 轮换取
  新连接）→ 重试，最多 3 次。
- **市场码自动推断**：6xx=沪(17)，其余=深(33)。
- **日内K bar 序号**：5分K 等 `time=None`、原 dt1 值存 `bar_index`（见 §14d 时间编码表）。

```python
from thspypc import THSClient
c = THSClient(user, pwd)
for period in ["day", "week", "month", "5min"]:
    recs = c.kline("000089", period=period, count=335)
    # recs: [{code, time, open, high, low, close, volume, amount, ...}]
```

**测试**：`tests/probe_kline.py`（用 kline() 连发多周期，可选 --timeline 测分时）。

#### 14g. 分时图请求（⏳ 请求格式已实现，待交易日盘后验证完整 240 根）

分时走与 K线**不同的子帧**（2026-07-24 抓包 pageid=9354 逐字节确认）：

| 维度 | K线（build_kline_query） | 分时（build_timeline_query） |
|------|--------------------------|------------------------------|
| 子帧类型[7:11] | `12 00 09 00`（0x0009） | **`12 00 02 00`（0x0002）** |
| 路由[11:13] | 0x0001/0x014e（随周期） | **0x000a** |
| pageid | 9355 | **9354** |
| DateTime | 周期码(偏移-0) | **8192(0-0)**（8192=0x2000 分时码） |
| ReqFuquan | 有（复权） | 无（分时实时不复权） |
| DataType | 7,8,9,11,13,19（OHLCV） | **10,13,14,15,19,22,23,54,6,45** |

**已实现** `build_timeline_query(code, market, ...)`（src/thspypc/protocol.py）。
响应解码复用 `parse_kline_hd3_response`（同 hd3.1 变体，字段表不同但结构一致）。

**验证状态**：请求格式正确（服务器接受，返回 `CodeListSize=0` 而非 RST），但非交易
时段/盘前测试时服务器无当日分时数据可返回（分时是交易日盘中实时生成，盘后才有完整
一份）。**待交易日盘后用 `tests/probe_kline.py --timeline` 验证完整 240 根分时**。

抓包样本里分时响应已确认可解（§14d 的 flag=0x0042 dc=33 帧，dt10=现价/dt49=量，
时间 14:55-15:30 当日盘尾片段）。

#### 14h. 历史分时（回忆）协议（✅ 请求+日期编码完全破解，响应解码部分完成）

**抓包产物**（2026-07-24，用户翻历史分时回忆 + 明确日期锚点）：
- `captures_live/timeline_20260724_090145.pcap`（第一次，指数 1A0002 + 紫光 000938）
- `captures_live/timeline_20260724_091133.pcap`（第二次，260515/260630 锚点）
- `captures_live/timeline_20260724_092405.pcap`（第三次，紫光 260513/14/0723 锚点）
- `tests/capture_timeline.py`（抓包脚本，识别 pageid=9354/4417 + DateTime=8192）

**核心发现**：历史分时与当日分时是**两套完全不同的协议**：

| 维度 | 当日分时（build_timeline_query） | 历史分时回忆（build_history_timeline_query） |
|------|--------------------------------|---------------------------------------------|
| pageid | 9354（或 4214） | **4417** |
| 子帧 | 单层 0x0002 路由 0x000a | **嵌套**（外层 0x0009 路由 0x017c [18]=0x20 + 内层 0x0002 路由 0x027c）|
| DataType | 10 个字段 | **26 个 level2 字段**（201-230 大单金额 = 分时图第二条"大单金额"曲线）|
| DateTime | `8192(0-0)` | `8192(<bar_start>-<bar_start+355>)` |
| DTPrevOff | 无 | `-61`（含义待确认，抓包恒为此值）|
| 响应 | hd3.1 flag=0x0042 | **hd1.0 嵌套壳** dc=0x0400xxxx |

**★ arg1（bar_start）的日期编码——完全破解（4 锚点验证）**：

`DateTime=8192(bar_start-bar_start+355)` 里的大数是该交易日**首条分时 bar 的全局
序号**，编码规则（关键突破来自 Mac 版 thspy 的 bar 序号思路 + 2048/日历日 规律）：

```
bar_start = (目标日期 - date(1849,4,5)).days × 2048 + 606
          = (目标日期.toordinal() - 675063) × 2048 + 606
```

- **日历日序号**（含周末，非交易日），每日 ×2048
- **日内偏移 606**（历史交易日恒定；当日盘前请求是 960）
- 组内差 arg2-arg1 恒 = **355**（一个交易日的分时点数，含集合竞价 padding）
- **arg1 只依赖日期、与股票无关**（000938 和 1A0002 同一天 arg1 完全相同）

**验证锚点**（arg1↔日期，全部精确匹配，差=0）：
```
2026-05-13 → 132477534    2026-06-30 → 132575838
2026-05-14 → 132479582    2026-07-23 → 132622942
```
（第一次抓包的 132371038 反推为 2026-03-22；今日盘前值 132627392 因 intra=960 反推
差 1 天，属正常——开盘后服务器会用真实收盘值）

**已实现**（`src/thspypc/protocol.py`）：
- `date_to_timeline_bar(date)` / `timeline_bar_to_date(bar)` —— 日期↔bar序号互转
- `build_history_timeline_query(code, date=..., market=..., ...)` —— 请求构造，
  **逐字节与抓包一致**（指数 1A0002 单票请求验证，差异 0 字节）
- `parse_history_timeline_response(body)` —— 响应解码框架

**响应解码状态**：
- ✅ **指数 1A0002 定长帧可解**：dc=0x040000f2（低16位=242 记录数），flag=0x007e，
  hs=88，fc=22，字段 dt1/10/13/19/22/23/201-230 全 width=4。记录区前 ~110B 壳头，
  242 条定长记录行主序。dt10 现价、dt13 量、dt19 额均正确解出。
- ⏳ **紫光 000938 等 level2 个股帧待解**：记录**变长**（93/94 字节交替，因 level2
  大单字段每 bar 档位数不同）。字段表 width 和 ≠ 固定 hs，需定位变长字段宽度规则。
  这是后续优化项，不影响请求侧使用。

**关键：hexin 实发的是「多票混合请求」**（一个 CodeList 含 399002+000938 等），
thspypc 的 `build_history_timeline_query` 构造的是**单票请求**（与指数 1A0002 抓包
结构一致，已验证）。单票请求服务器是否接受待实发验证（盘后即可测历史分时，不需开盘）。

**待办**：
1. 实发验证单票历史分时请求（`THSClient.history_timeline(date)`）
2. 紫光 level2 变长帧解码
...
#### 14i. K线复权确认 + 价格舍入精度（✅ 2026-07-24 实测确认）

**复权是生效的**（用户用紫光 000938 对比同花顺客户端确认）：
- `build_kline_query` 默认 `fuquan="Q"`（前复权），请求文本含 `ReqFuquan=Q`
- 月K 1999-11-30（紫光上市月）实测：**前复权 O=2.98**（不复权原始价≈9.01），
  证明服务器按 ReqFuquan=Q 返回了前复权数据
- 日K/周K/月K/5分K 末根收盘价一致（42.12），走势与同花顺客户端吻合

**0.01 价格误差的根因——Python 银行家舍入（非复权问题）**：

用户对比发现"几个值差 0.01"。实测根因：服务器返回的价格精度到**厘**（如 31.845），
同花顺客户端用**四舍五入**显示（31.85），而 Python `:.2f` / `round()` 用**银行家舍入**
（round half to even），31.845 → 31.84（4 是偶数）。差异只在 `.xx5` 边界值出现。

**修复**：显示层用 `Decimal + ROUND_HALF_UP`（四舍五入）替代 `:.2f`：
```python
from decimal import Decimal, ROUND_HALF_UP
def r2(v): return str(Decimal(str(v)).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP))
```
已应用于 `tests/probe_kline_recent.py`。`kline()` 返回值保持高精度（不 round），
避免涨跌幅计算/回测累积误差。

#### 14j. K线首个请求偶发失败 + 分帧截断 + 坏 IP（✅ 2026-07-24 定位修复）

实测现象（`probe_kline.py --code 000938` 连发 day/week/month/5min）：
- **day（首个请求）偶发 timeout**（`TimeoutError: timed out`），重试 1-2 次后成功
- 偶发**数据截断**：day 只拿 121/336 根、week 26 根、month 7 根（5min 始终完整 336）

**根因定位（三个独立问题，2026-07-24 IP 日志最终澄清）**：

1. **init 握手残留帧**：`_send_init_handshake` 旧实现只读 1 帧 + 0.2s peek，配置帧
   后的残留半帧污染首个 K线请求的 `read_frame`（扫 magic 错位）→ timeout。
   **修复**：init 握手循环排空（最多 8 帧，每帧 0.3s 超时），彻底清除残留。

2. **响应分帧截断（数据不全的根因）**：`_kline_query_once` 旧实现 `return recs`
   在**首个** hd3.1 数据帧就返回。当服务器把大 K线（day 336 根）**分多个 hd3.1 帧**
   返回时，只拿到第一帧（如 121 根）。
   **修复**：改为**累积合并**——读到首帧后继续读后续 hd3.1 帧合并记录，拿到数据后
   缩短超时（2s）快速确认无后续帧，直到读到非 hd3.1 帧（下个请求 ACK）或超时。

3. **★ 坏 IP（K线大请求的真正根因）**：加 IP 诊断日志后，4 次运行的 IP 对照铁证：

   | 连到的 IP | day 结果 |
   |----------|---------|
   | `8.134.123.179` / `8.134.86.216` / `8.134.121.153` / `8.132.233.143` | ✅ 首次成功 336 根 |
   | `116.63.40.25`（ConnectionError）/ `116.63.85.120`（timeout）/ `122.9.202.190`（timeout） | ❌ 失败 |

   **116.63.x.x 段全军覆没**，第 4 次同进程内 116.63.85.120 + 122.9.202.190 都 timeout、
   换到 8.132.233.143 立即成功——彻底排除网络/账号问题，是**特定 IP 不支持大 K线查询**
   （只返回部分数据或 timeout）。5min 始终完整因其响应小、所有 IP 都能处理。
   **修复**：`_bad_kline_ips` 黑名单——K线失败时记录 IP，connect 选 batch 时跳过
   （`good_ips = [ip for ip in sorted_ips if ip not in self._bad_kline_ips]`）。
   实例级（单进程内累积），不跨进程（避免误伤恢复的 IP）。

   **★ 数据不全检测（关键补充）**：坏 IP 有两种表现——① 直接 timeout/ConnectionError
   （好检测）；② **"成功"返回但只给最近部分数据**（day 121/336、week 26、month 7，
   首根都在 ~半年前，不抛异常）。第二种必须主动检测：拿到数据后校验
   `len(recs) < count * 0.5` 则判为坏 IP，加黑名单换 IP 重试。新股天然根数少但
   day/week/month 都少，不会误触发（阈值相对 count）。

   实测坏 IP 段：`116.63.x.x`、`119.3.x.x`、`122.9.202.190` 等；好 IP 段：`8.134.x.x`、
   `8.132.x.x`、`8.145.x.x`、`122.9.207.158` 等。

**read_frame 容错改进**（kline + history_timeline）：
- `ValueError`（magic 错位/半帧）：丢一段 `recv(8192)` 后继续找下个帧边界，不再直接
  让连接失败
- 拿到数据后排空残留，避免污染下个周期请求

**IP 诊断**：`_connected_ip` 属性记录当前连接 IP，kline 失败日志带 `IP=x.x.x.x`，
`probe_kline.py` 启用 INFO 日志可见 connect/fail 的 IP。
