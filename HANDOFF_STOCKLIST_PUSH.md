...
#### 14d. 响应格式（⚠️ 框架已破，卡在位平面重组——0x0042 变体）

K线响应是 **hd3.1 的 `flag=0x0042` 变体**（区别于排序榜单的 `0x0100`）。
2026-07-24 集中逆向，框架已摸清，但**最后一步的位平面重组未突破**，记录如下
避免重复踩坑。

**已确认的帧结构**（抓包 + BitRLE 解码验证）：
```
hd3.1\0
+ dc(LE32)           ← 记录数（5分K=2147 根）
+ flag(LE16=0x0042)  ← ★ 0x0042 变体标记（注意：0x0100 是排序变体）
+ fc(LE16)           ← 字段数（K线=7）
+ 字段表(fc×4B)      ← (dt,fmt,flags,width)，K线全 fmt=0x70 width=4
+ 26字节壳头          ← 16 00 01 00 21 + ASCII代码(000088) + padding
+ BitRLE头(BE32=dc*hs) ← ★ 大端，值=dc×hs（5分K=60116）
+ BitRLE位流
```

⚠️ **头格式与 0x0100 不同**：0x0042 没有 0x0100 的 `unk` 字段，`fc` 紧跟 `flag`；
字段表后多 26 字节壳头（含股票代码）。但 BitRLE 流本身用现有
`_decode_bitrle_0x13746d0` 能解，**输出正好 dc×hs 字节**（精确匹配，非巧合）。

**字段表**（K线，fmt=0x70=THS float，width=4）：
| dt | 含义 | dt | 含义 |
|----|------|----|------|
| 1 | 时间 | 11 | 收盘价 |
| 7 | 开盘价 | 13 | 成交量 |
...
**❌ 卡点：BitRLE 输出的位平面重组**（已排除多种方案）

BitRLE 解出的 `dc×hs` 字节，按各种排列方式重组后，THS float 解码均得哨兵值
（255/65535/0xFFFFFFFF）或位模式值，无真实 OHLC。已试并排除：

| 重组方式 | 结果 | 排除理由 |
|---------|------|---------|
| 直接行主序（不转置）| 全 `-447392.42` 重复 | 固定字节模式，解码错 |
| 字段主序（[dt1全部][dt7全部]...）| 全 0 | 字段间无分隔依据 |
| 整体列优先转置 (nrows=dc,ncols=hs) | 时间有规律但 OHLC 全 65535 | 时间像对了但价格是哨兵 |
| 字段内列优先（每字段 dc×4 独立转置）| 351 根"合理"但全 255 | 假阳性（255≤65535 恒成立）|

字节分布异常：`0x00` 占 30683/60116（**一半！**）、`0xff` 占 10224、`0xaa` 占 368。
`0x00` 占一半强烈暗示 **BitRLE 输出是按 bit 平面组织的**（每个 bit 位的所有值连续
存储），需要 **bit 级重组**而非字节级转置。这与 Mac hx1.3（字节级列优先即可恢复）
不同——0x0042 可能是更底层的位平面编码。

⚠️ **Mac 版帮不上忙**：Mac K线走 `_parse_hx12`（hx1.2，纯明文 LE24÷10000，非压缩），
与 PC 的 hd3.1+0x0042 是完全不同的容器和编码。Mac 的转置参数（hx1.3 专用）不适用。

**下一步突破点**（按可行性排序）：
1. **路径 C：动态逆向**（最可靠）——用 x32dbg 在 hexin.exe 的 BitRLE 解码函数下断，
   dump 解码后缓冲区，和 thspypc 输出对比，确定差在哪一步。需装 ScyllaHide 绕反调试
  （文档 §6b 有操作手册，`stockname_%s_%d.txt` 写文件断点法同款）。0x0042 的解码函数
   地址待定位（可在 hexin_unpacked.exe 搜 hd3.0/hd3.1 字符串 xref）。
2. **路径 B 升级：bit 级穷举**——把 BitRLE 输出当作 bit 流，尝试按 bit 平面重组
  （8 种 bit 序 × 字段分块/不分块），用金融约束（开≤高、开≥低、低≤收≤高、价格 1~10000）
   自动验证。组合空间比字节级大，成功率中等。
3. 暂搁置，等更多 0x0042 样本（不同股票/周期）做差分对照。

**当前可用度**：请求格式完全可用（能构造 K线请求拿响应），响应解码待突破。
建议 K线 API 先做「请求可用」版，响应解码标记实验性。
